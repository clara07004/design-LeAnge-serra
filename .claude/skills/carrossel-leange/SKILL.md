---
name: carrossel-leange
description: >
  Cria carrosséis completos para Instagram e TikTok com a identidade visual da LeAnge.
...
# /carrossel-leange — Criação de Carrossel
...
- **Geração de imagem (default):** `.claude/skills/gpt-image2-leange/gerar-imagem.py` + `credentials/openai_key.txt`
- **Geração de imagem (fallback):** `.claude/skills/nanobanana-leange/` (Gemini, grátis — se GPT falhar)
...
python ".claude/skills/publicar-social-leange/validar-dimensao.py" "CAMINHO_ABSOLUTO/instagram" 1080 1350

# 2) Overflow interno — nenhum texto pode ultrapassar as bordas do canvas
node ".claude/skills/publicar-social-leange/validar-overflow.js" "CAMINHO_ABSOLUTO/instagram"
```
...
- `validar-overflow.js` renderiza cada slide e acusa qualquer elemento de texto que sangre para
  fora de 1080×1350 (caso clássico: info bar / rodapé horizontal cortado no canto). Ele aponta o
...
- Contexto: usar o perfil de público definido em `_contexto/empresa.md` (setor, linguagem do público)
...
- **Slide 1 (Capa):** SEMPRE — obrigatório, sem exceção. Cadeia de tentativas: GPT Image 2 → Nanobanana → image-gen-leange. Se um motor falhar, sinalizar ao usuário e tentar o próximo sem interromper o fluxo. Só parar se todos os três falharem.
...
- Descrever a cena concreta relacionada ao conteúdo do slide (ex: "a happy dog relaxing by the pool at a cozy pet-friendly inn")
...
- Incluir contexto da experiência LeAnge Serra quando relevante: `pet-friendly hotel`, `dogs`, `cozy mountain inn`, `nature`, `fireplace`
...
```powershell
python ".claude/skills/gpt-image2-leange/gerar-imagem.py" "PROMPT" "conteudo/carrosseis/PERIODO/DIA/instagram/img-slideXX.png" "portrait"
```
...
- Se o script falhar (exit code ≠ 0):
  ...
  - **Fallback Nanobanana:** usar `.claude/skills/nanobanana-leange/` com o mesmo prompt (se a skill estiver instalada)
...
- **Barras horizontais (info bar / rodapé de informações) — ponto de falha recorrente.** Uma fileira de
  ...
  - no máximo ~4 itens curtos; se precisar de mais, encurtar os rótulos (ex.: "10 suítes", não "Pousada com 10 suítes pet-friendly disponíveis");
...
- Sem labels de seção ("COMODIDADES", "LOCALIZAÇÃO")
- Sem eyebrows ("COMPARATIVO", "SAIBA MAIS")
